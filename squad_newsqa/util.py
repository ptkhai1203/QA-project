...
def eval_dicts(gold_dict, pred_dict, no_answer):
    avna = f1 = em = total = 0
    mdict = {'key': [], 'EM':[], 'f1':[], 'context': [], 'ques':[], 'pred':[], 'ground':[]}
    for key, value in pred_dict.items():
        total += 1
        ground_truths = gold_dict[key]['answers']
        prediction = value
        mem = metric_max_over_ground_truths(compute_em, prediction, ground_truths)
    ...
    eval_dict = {'EM': 100. * em / total,
                 'F1': 100. * f1 / total}
    my_file = '/content/drive/MyDrive/3RD/QA/squad/save/track/pred.csv'
