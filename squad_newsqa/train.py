...
from models import LSTMQA, BLSTMQA, ALSTMQA
...
def main(args):
    ...
    def filter_fn(dataset):
        ...
        for x in dataset:
            if x[4] != 0 and x[5] != 0:
              res.append(x)
        return res
    train_dataset = filter_fn(train_dataset)
    print('len filtered dataset: ', len(train_dataset))
    ...
    dev_dataset = filter_fn(dev_dataset)
