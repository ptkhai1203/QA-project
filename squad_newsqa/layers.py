...
class LSTMEncoder(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        # Apply LSTM
        x, _ = self.lstm(x)  # (batch_size, seq_len, 2 * hidden_size)
...
class BLSTMEncoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, drop_prob=0.):
        ...
        self.drop_prob = drop_prob
        self.blstm = nn.LSTM(input_size, hidden_size, num_layers,
                            batch_first=True,
                            bidirectional=True,
                            dropout=drop_prob if num_layers > 1 else 0.)
...
class BiDAFAttention(nn.Module):
    ...
    def forward(self, c, q, c_mask, q_mask):
        ...
        # x = torch.cat([c, a, c * a], dim=2) # (bs, c_len, 3 * hidden_size)
...
class LSTMOutput(nn.Module):
    ...
    def forward(self, cont, mask):
        logits_1 = self.linear_1(cont)
        logits_2 = self.linear_2(cont)

        log_ps = masked_softmax(logits_1.squeeze(), mask, log_softmax=True)
        log_pe = masked_softmax(logits_2.squeeze(), mask, log_softmax=True)

        return log_ps, log_pe

class ALSTMOutput(nn.Module):
    def __init__(self, hidden_size, drop_prob):
        super(ALSTMOutput, self).__init__()
...
class BLSTMOutput(nn.Module):
    def __init__(self, hidden_size, drop_prob):
        super(BLSTMOutput, self).__init__()
