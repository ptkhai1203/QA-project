...
def build_features(args, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    # load data
    # if data_type == 'train' and os.path.isfile(out_file):
    #     with np.load(out_file, allow_pickle=True) as data:
    #         o_context_idxs = data['context_idxs']
    #         o_context_char_idxs = data['context_char_idxs']
    #         o_ques_idxs = data['ques_idxs']
    #         o_ques_char_idxs = data['ques_char_idxs']
    #         o_y1s = data['y1s']
    #         o_y2s = data['y2s']
    #         o_ids = data['ids']
    #     context_idxs = np.concatenate([o_context_idxs, context_idxs],axis=0)
    #     context_char_idxs = np.concatenate([o_context_char_idxs, context_char_idxs],axis=0)
    #     ques_idxs = np.concatenate([o_ques_idxs, ques_idxs], axis=0)
    #     ques_char_idxs = np.concatenate([o_ques_char_idxs, ques_char_idxs], axis=0)
    #     y1s = np.concatenate([o_y1s, y1s], axis=0)
    #     y2s = np.concatenate([o_y2s, y2s], axis=0)
    #     ids = np.concatenate([o_ids, ids], axis=0)
    np.savez(out_file,
